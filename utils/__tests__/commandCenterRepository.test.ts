...
describe("commandCenterRepository", () => {
  ...
  it("uses a distinct owner cache key", async () => {
    ...
    const repository = createCommandCenterRepository({
      cache: {
        get: (key) => {
          cacheGets.push(String(key));
          return null;
        },
        set: (key) => {
          cacheSets.push(String(key));
        },
      },
      fetchSheetRangeObjects: async (range) => {
        if (range.startsWith("Analytics_KPI_Daily")) {
    ...
    await repository.getOwnerPayload("YTD");

    expect(cacheGets).toContain("commandCenter.owner.YTD");
    expect(cacheSets).toContain("commandCenter.owner.YTD");
    expect(cacheGets).not.toContain("commandCenter.manager.YTD");
  });
});
